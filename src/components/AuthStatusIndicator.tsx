...
const AuthStatusIndicator: React.FC = () => {
    ...
    const initAuth = async () => {
      try {
        ...
        unsubscribe = auth.onAuthStateChanged((user) => {
          ...
          const log = `[${timestamp}] Auth changed: ${user ? `${user.uid} (${user.isAnonymous ? 'anon' : 'google'})` : 'null'}`;
  ...
  return (
    <div className="fixed top-2 right-2 z-50">
      ...
      >
        {!isReady ? '⏳' : user && !user.isAnonymous ? '🔐' : user ? '👤' : '❌'} 
        Auth
      </div>
      ...
      {showDetails && (
        <div className="absolute top-8 right-0 bg-white border rounded-lg shadow-lg p-3 w-64 text-xs">
          <div className="mb-2">
            <strong>Status:</strong> {!isReady ? 'Loading...' : user ? 'Authenticated' : 'Not authenticated'}
          </div>
          {user && (
            <div className="mb-2 space-y-1">
              <div><strong>UID:</strong> {user.uid.substring(0, 8)}...</div>
              <div><strong>Type:</strong> {user.isAnonymous ? 'Anonymous' : 'Google'}</div>
